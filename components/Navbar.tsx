...
const Navbar: React.FC<NavbarProps> = ({ onNavigate, currentPage, onBookNow, onSignIn, user }) => {
  ...
  return (
    <nav 
      className={`${isDark ? 'relative bg-white/80 backdrop-blur-xl border-b border-slate-100 shadow-sm sticky top-0' : 'absolute top-0 left-0 right-0'} z-[100] px-4 md:px-12 py-3 md:py-4 flex items-center justify-between transition-all duration-500`}

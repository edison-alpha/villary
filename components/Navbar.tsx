...
import { Menu, Search, Heart, LogIn } from 'lucide-react';
...
const Navbar: React.FC<NavbarProps> = ({ onNavigate, currentPage, onBookNow, onSignIn, user }) => {
  ...
  return (
    <nav className={`${isDark ? 'relative bg-white/80 backdrop-blur-xl border-b border-slate-100 shadow-sm sticky top-0' : 'absolute top-0 left-0 right-0'} z-[100] px-6 py-4 md:px-12 flex items-center justify-between transition-all duration-500`}>
      {/* Logo & Brand */}
      <div className="flex items-center gap-8">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => onNavigate('home')}>
          <span className={`${isDark ? 'text-[#0d5c63]' : 'text-white'} text-2xl font-serif font-bold tracking-tight`}>
            Villays
          </span>
        </div>
      ...
      <div className="flex items-center gap-6">
        ...
        {user ? (
          ...
          >
            ...
            <div className="w-10 h-10 rounded-full border-2 border-[#0d5c63] overflow-hidden p-0.5 group-hover:scale-105 transition-transform">
          ...
          <button 
            onClick={onSignIn}
            className={`flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest px-6 py-2.5 rounded-full border transition-all ${isDark ? 'border-[#0d5c63] text-[#0d5c63] hover:bg-[#0d5c63] hover:text-white' : 'border-white text-white hover:bg-white hover:text-slate-900'}`}
          >
            <LogIn size={16} /> Sign In
          </button>
        )}
        
        <button className="lg:hidden p-2 text-current"><Menu size={24} /></button>
      </div>
    </nav>
  );
};
